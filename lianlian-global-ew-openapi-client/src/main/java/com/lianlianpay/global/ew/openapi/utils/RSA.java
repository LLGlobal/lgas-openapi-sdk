...
public class RSA {
     ...
     */
    public static Pair<String, String> generateKeyPair(int length) {
        if (length < 1024) {
            throw new IllegalArgumentException("length must be at least 1024 bits");
        }
        try {
            KeyPairGenerator kpg = KeyPairGenerator.getInstance("RSA");
            kpg.initialize(length);
            KeyPair kp = kpg.generateKeyPair();
            return new Pair<>(Base64.getEncoder().encodeToString(kp.getPublic().getEncoded()), Base64.getEncoder().encodeToString(kp.getPrivate().getEncoded()));
        } catch (Exception e) {
    ...
    public static void generateKeyPairThenExport(int length, String publicKeyPath, String privateKeyPath) throws IOException {
        Pair<String, String> pair = generateKeyPair(length);
        try (OutputStream pub = new FileOutputStream(publicKeyPath);
             OutputStream pri = new FileOutputStream(privateKeyPath)) {
            pub.write(convertPublicKeyToPEMFormat(pair.getKey()).getBytes());
            pri.write(convertPrivateKeyToPEMFormat(pair.getValue()).getBytes());
        }
    }
